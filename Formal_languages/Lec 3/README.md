...
матрица a матрица В. перемножаем - получаем, что каждый элемент а умножен на матрицу В. Используем для построения матрицы 
...
Перемножаем все еще с помощью кронакера, но раньшше элементом было пересечение множеств, а теперь - булево пересечение, посчитать которое гораздо проще
...
* Транзитивное замыкание ищет достижимость для всех пар вершин. А что если нужно только для определенной пары? Есть три варианта
...
* граф ориентированный, пока без меток на ребрах. мета-шаг: сменить все вершины с front'a  на другие
